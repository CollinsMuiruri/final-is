...
def signup(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			login(request, user)
			messages.success(request, 'Success! Signup was a success, now join or create a neighbourhood to start using Neighbourhood Watch')
...
def profiles(request):
	'''
	This view function will fetch all user's profiles
	'''
	profiles = Profile.objects.filter(pk = request.user.join.hood_id.id)
	return render(request,'accounts/allprofiles.html',{"profiles":profiles})
...
def searchForum(request):
	...
	if request.GET['searchForum']:
		...
		hood = Neighbourhood.objects.get(pk = request.user.join.hood_id.id)
		businesses = Business.objects.filter(hood = request.user.join.hood_id.id)
		return render(request,'posts/search.html',{"message":message,"businesses":businesses,"hood":hood})
